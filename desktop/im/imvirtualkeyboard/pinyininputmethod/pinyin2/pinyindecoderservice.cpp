...
bool PinyinDecoderService::init()
{
    ...
    if (!QFileInfo::exists(sysDict)) {
        sysDict = QLatin1String("/usr/share/pinyin/3rdparty/pinyin/data/dict_pinyin.dat");
        if (!QFileInfo::exists(sysDict))
            sysDict = QLibraryInfo::location(QLibraryInfo::DataPath) + QLatin1String("/pinyin/dict_pinyin.dat");
    }
    ...
    //qDebug()<<"setUserDictionary";
    this->setUserDictionary(true);

    initDone = im_open_decoder(sysDict.toUtf8().constData(), usrDictInfo.absoluteFilePath().toUtf8().constData());
    if (!initDone){
        qDebug() << "Could not initialize pinyin engine. sys_dict:" << sysDict << "usr_dict:" << usrDictInfo.absoluteFilePath();
    }
